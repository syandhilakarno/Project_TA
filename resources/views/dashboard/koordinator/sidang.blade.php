...
<script src="https://code.jquery.com/ui/1.13.2/jquery-ui.min.js"></script> @section('content')
<div class="container">
    <div class="sidebar">
    ...
    <div class="main">
        ...
        <div class="content">
            ...
            <div class="search-bar mb-3">
                <input type="text" id="search" placeholder="Cari Mahasiswa, NIM, Judul..." class="form-control" onkeyup="searchTable()">
            </div>

            <div class="table-responsive-wrapper">
                <table id="sidangTable" class="">
                    ...
                    <tbody>
                        ...
                        <form action="{{ route('koordinator.updateSidang', $item->id) }}" method="POST">
                            ...
                            <tr>
                                ...
                                <td>{{ optional(optional($item->mahasiswa)->user)->name ?? 'N/A' }}</td>
                                ...
                                <td class="text-center align-middle">
                                    <button type="submit" class="btn btn-success btn-sm" title="Simpan Baris">
                        ...
                        @empty
                        <tr>
                            <td colspan="10" class="text-center">Belum ada data sidang.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
...
$(document).ready(function() {
    ...
    // Inisialisasi Datepicker (format YYYY-MM-DD agar dibaca HTML)
    ...
    function syncToChecked(selector, value) {
        $('.sidang-checkbox:checked').each(function() {
            ...
            // Cek jika target adalah Select2
    ...
    // Event listeners untuk sinkronisasi
    $('.tanggal-sidang').on('change', function() {
        if ($('.sidang-checkbox:checked').length > 1) {
            syncToChecked('.tanggal-sidang', $(this).val());
        }
    });
    $('.ketua-select').on('change', function() {
        if ($('.sidang-checkbox:checked').length > 1) {
            syncToChecked('.ketua-select', $(this).val());
        }
    });
    $('.penguji-select').on('change', function() {
        if ($('.sidang-checkbox:checked').length > 1) {
            syncToChecked('.penguji-select', $(this).val());
        }
    });
    $('.penguji2-select').on('change', function() {
        if ($('.sidang-checkbox:checked').length > 1) {
            syncToChecked('.penguji2-select', $(this).val());
        }
    });
    $('.ruang-sidang').on('change', function() {
        if ($('.sidang-checkbox:checked').length > 1) {
            syncToChecked('.ruang-sidang', $(this).val());
        }
    });

    // Simpan Semua yang Dipilih (Bulk)
    $('#saveSelected').on('click', function() {
        ...
        $('.sidang-checkbox:checked').each(function() {
            ...
            selected.push({
                id: id,
                tanggal_sidang: $('input.tanggal-sidang[data-sidang-id="'+id+'"]').val(),
    ...
    // Tooltip untuk nama dosen di Select2
